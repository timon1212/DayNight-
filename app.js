...
window.addEventListener("DOMContentLoaded",async()=>{
  ...
  // tab navigation already handled in HTML script
});
...
async function distributeStockUI(){ 
  ...
  const routes=await getAll("routes");
  const route=routes.find(r=>r.id==document.getElementById("routeSelect").value);
  if(!route) return;
...
// ================= FINANCE / GAS =================
async function recordFinanceUI(){ const routes=await getAll("routes"); const route=routes.find(r=>r.id==document.getElementById("routeSelect").value); if(!route) return; const total=route.pins.reduce((sum,s)=>sum.revenue+Object.values(s.stockUsed).reduce((a,b)=>a+b*0.5,0),0); await addData("finance",{route:route.name,total,date:new Date().toLocaleString()}); alert("Finance Recorded"); displayFinance();}
...
async function displayFinance(){ const finance=await getAll("finance"); const div=document.getElementById("financeList"); div.innerHTML=finance.map(f=>`Route: ${f.route} | Total: $${f.total.toFixed(2)} | Date: ${f.date}`).join("<br>"); }
