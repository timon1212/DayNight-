...
document.getElementById('submitFormBtn').addEventListener('click',async()=>{
  ...
  if(!name||isNaN(age)) return alert('Name and age required');
  await addData('forms',{name,age,notes,image:imageData,date:new Date().toLocaleString()});
  document.getElementById('formResults').innerHTML=`Form submitted for ${name}`;
